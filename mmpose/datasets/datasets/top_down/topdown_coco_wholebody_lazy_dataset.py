...
from ...pipelines.loading import LoadImageFromlmdb



@DATASETS.register_module()
class TopDownCocoWholeBodyLazyDataset(TopDownCocoDataset):
    ...
    def load(self):
        ...
        if isinstance(self.pipeline.transforms[0], LoadImageFromlmdb):
            results = copy.deepcopy(self.db[0])
            results['ann_info'] = self.ann_info

            self.pipeline.transforms[0].load(results)
        print(f'=> num_images: {self.num_images}')
        print(f'=> load {len(self.db)} samples')

        print(f"On process {os.getpid()}")
    ...
    def unload(self):
        if self.loaded:
            ...
            del self._class_to_ind
            del self._class_to_coco_ind
            del self._coco_ind_to_class_ind
            del self.img_ids
            del self.num_images
            del self.id2name
            del self.name2id
            ...
            self.loaded = False
            if isinstance(self.pipeline.transforms[0],LoadImageFromlmdb):
                self.pipeline.transforms[0].unload()
            gc.collect()



            print(f'=> unloaded: {self.annotations_path}')
    ...
    def __init__(self,
        ...
        self.loaded = False
        print(f"Built dataset: {ann_file}")
    @staticmethod
    ...
    def _load_coco_keypoint_annotations(self):
        ...
        for img_id in self.img_ids:
            #print(img_id,len(gt_db))

            gt_db.extend(self._load_coco_keypoint_annotation_kernel(img_id))
        #print([item["image_file"] for item in gt_db[:10]])
        return gt_db
